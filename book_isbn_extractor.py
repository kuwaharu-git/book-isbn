...
class ImageProcessor:
    ...
    def read_barcodes(self, image_path: str) -> List[str]:
        """Detect and decode barcodes from image, return list of decoded strings."""
        try:
            ...
            logger.error(f"Error reading barcodes from {image_path}: {str(e)}")
            return []
...
class ISBNExtractor:
    ...
    def __init__(self):
        # ISBN regex patterns - ISBN表記がある場合のみ抽出
        self.isbn_patterns = [
            r"ISBN[\s\-:]*(\d{3}[\s\-]*\d{1}[\s\-]*\d{3}[\s\-]*\d{5}[\s\-]*\d{1})",  # ISBN-13 (ISBN表記必須)
            r"ISBN[\s\-:]*(\d{1}[\s\-]*\d{3}[\s\-]*\d{5}[\s\-]*[\dX])",  # ISBN-10 (ISBN表記必須)
        ]

    def extract_isbns_from_barcodes(self, barcodes: List[str]) -> List[str]:
        """Extract ISBN-13 numbers from barcode results (978/979で始まる13桁のみ)。"""
        isbns = []
        for code in barcodes:
            # ISBN-13: 978または979で始まる13桁
            if re.fullmatch(r"97[89]\d{10}", code):
                if self._validate_isbn13(code):
                    isbns.append(code)
...
class BookAPIClient:
    ...
    def __init__(self, api_delay: float = 1.0):
        ...
        self.session.headers.update({"User-Agent": "Book-ISBN-Extractor/1.0"})
    ...
    def get_book_info(self, isbn: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(
                f"Error retrieving book info for ISBN {isbn}: {str(e)}"
            )
            return None
        ...
        try:
            ...
            if data.get("totalItems", 0) > 0:
                ...
                return {
                    ...
                    "authors": ", ".join(
                        volume_info.get("authors", ["Unknown"])
                    ),
                    ...
                    "description": (
                        volume_info.get("description", "")[:500] + "..."
...
class BookISBNExtractor:
    ...
    def process_folder(
        self, folder_path: str, output_csv: str = "book_information.csv"
    ) -> None:
        ...
        for i, image_file in enumerate(image_files, 1):
            logger.info(
                f"Processing image {i}/{len(image_files)}: {os.path.basename(image_file)}"
            )

            # バーコードからISBN候補を抽出
            barcodes = self.image_processor.read_barcodes(image_file)
            logger.info(
                f"Barcodes found in {os.path.basename(image_file)}: {barcodes}"
            )
            isbns = self.isbn_extractor.extract_isbns_from_barcodes(barcodes)
            if not isbns:
                logger.info(
                    f"No ISBNs found in {os.path.basename(image_file)}"
                )
        ...
        for i, (isbn, source_files) in enumerate(all_isbns.items(), 1):
            if self.skip_api:
                logger.info(
                    f"Skipping API call for ISBN: {isbn} (offline mode)"
                )
                book_data.append(
                    {
                        "isbn": isbn,
            ...
            logger.info(
                f"Retrieving book information {i}/{len(all_isbns)} for ISBN: {isbn}"
            )
            ...
            if book_info:
                ...
                logger.info(
                    f"✓ Found book: {book_info.get('title', 'Unknown')}"
                )
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Extract ISBN information from images and retrieve book details"
    )
    parser.add_argument(
        "folder_path", help="Path to folder containing image files"
    )
    parser.add_argument(
        "-o",
        "--output",
        default="book_information.csv",
    ...
    extractor = BookISBNExtractor(
        api_delay=args.api_delay, skip_api=args.skip_api
    )
